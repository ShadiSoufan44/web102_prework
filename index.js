...
function addGamesToPage(games) {
  for (let i = 0; i < games.length; i++) {
    ...
    // about each game
    gameCard.innerHTML = `
            <h3>${games[i].name || "Unknown Title"}</h3>
            <img src="${games[i].img || "default-image.jpg"}" alt="${
      games[i].name || "Game Image"
    }" class="game-img">
            <p>Description: ${
              games[i].description || "No description available"
            }</p>
            <p>Backers: ${games[i].backers || 0}</p>
            <p>Pledged: $${(games[i].pledged || 0).toLocaleString()}</p>
        `;
...
// use the length property of the array to get the number of games
...
// Create a template string using the ternary operator for grammatical correctness
const descriptionString = `
    So far, we've raised $${totalraised.toLocaleString()} for ${totalgames} games. ${
